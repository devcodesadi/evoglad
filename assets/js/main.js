...
//-------------------------Key Feature section fade-in animation------------------//
...
// Tab switching with animation delay
document.querySelectorAll('[data-bs-toggle="pill"]').forEach((tab) => {
  tab.addEventListener("shown.bs.tab", function (e) {
    // Reset and re-trigger animations for cards in the new tab
...
// Benefit section (SuperCharge Customer)

class BenefitCarousel {
  constructor() {
    this.currentSlide = 0;
    this.totalSlides = 7;
    this.autoPlayInterval = 4000; // 4 seconds
    this.progressInterval = 100; // Update progress every 100ms
    this.isPlaying = true;
  ...
  updateProgress() {
    if (this.isPlaying) {
      this.progressWidth +=
        100 / (this.autoPlayInterval / this.progressInterval);
...
document.addEventListener("DOMContentLoaded", () => {
  window.benefitCarousel = new BenefitCarousel();
});

// Global functions for button clicks
function changeSlide(direction) {
  if (window.benefitCarousel) {
    window.benefitCarousel.changeSlide(direction);
  }
}

function goToSlide(index) {
  if (window.benefitCarousel) {
    window.benefitCarousel.goToSlide(index);
  }
}
